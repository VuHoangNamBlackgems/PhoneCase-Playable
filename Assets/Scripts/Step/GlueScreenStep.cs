...
public class GlueScreenStep : StepBase
{
   // ===============================================================
    //  REFS
    // ===============================================================
    ...
    [Tooltip("Null = dùng world; gán Transform để dùng trục up của object đó")]
    ...
    #region PROGRESS
    [Header("=== Progress ===")]
    [SerializeField] private RenderTexture glueRT;          // trống -> dùng glueCam.targetTexture
    ...
    private enum ProgressSource { Alpha, Luminance, MaxRGBA }
    ...
    [Header("Auto Fill (no shader)")]
    [Tooltip("Tự fill dần khi đủ ngưỡng")]
    [SerializeField] private bool animateFillOnComplete = true;
    [SerializeField] private float fillDuration = 0.6f;
    public enum FillMode { Horizontal, Vertical }
    ...
    {
        DOVirtual.DelayedCall(1f, ()=>glueTool.gameObject.SetActive(true));
        caseCollider = phoneCase.ScreenCollider;
        phoneCase.GlassNewScreen.gameObject.SetActive(true);
        ...
        {
            glueTool.gameObject.SetActive(true);
        });
    ...
    private void HandlePointer(out bool hasPointer, out Vector3 pointer)
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            { _dragging = false; hasPointer = false; pointer = default; return; }
            _dragging = true; _activeFingerId = -1;
        }
        ...
        else
        {
            for (int i=0;i<Input.touchCount;i++)
            {
                ...
                { hasPointer = false; pointer = default; if (t.phase==TouchPhase.Canceled||t.phase==TouchPhase.Ended) _dragging=false; return; }
    ...
    private void Progression_Tick()
    {
        ...
        if (!_autoFilled && (_targetProgress >= completeThreshold || _progress >= completeThreshold))
        {
            ...
            {
                if (_fillCo != null) StopCoroutine(_fillCo);
                _fillCo = StartCoroutine(AnimateFillRT_NoShader(rt, fillDuration, fillMode, invertFill, completeAfterFill));
        ...
        var src = SrcRT; if (src == null) return;
    ...
    {
        var rt = SrcRT; if (rt) Graphics.Blit(Texture2D.whiteTexture, rt);
    }
