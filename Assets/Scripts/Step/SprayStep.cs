...
public class SprayStep : StepBase
{
    // ============================ Refs ============================
    [Header("Refs")]
    [SerializeField] private Camera worldCam;            // Camera 3D
    ...
    SprayPreview currentSprayPreview;
    ...
    // ===== Done pulse runtime =====
    private bool _sprayedDuringDrag = false;  // lần kéo vừa rồi có xịt trúng case?
    private bool _armedIdlePulse = false;     // đã gài chờ idle sau khi thả tay
    private bool _idlePulseActive = false;    // đang chạy tween nháy nút
    private float _lastInteractionTime = 0f;  // mốc tương tác cuối
    ...
    private void Awake()
    {
        if (PlayerPrefs.GetInt("Tutorial") == 0)
        {
            ...
            UserTracking.TutorialAction(ActionTut.spray_art);
        }
        else
        {
    ...
    {
        var itemUnlocks = sprayPreviews.FindAll(x => x.IsUnlock == true);
        var itemRandom = sprayPreviews[UnityEngine.Random.Range(0, itemUnlocks.Count)];
        var data = itemRandom.SprayData;
      //  CustomerOrderManager.instance.OrderItem(new CustomerOrderManager.OrderEntry(data.id, data.icon));
        itemRandom.transform.SetAsFirstSibling();
        OnSelectSpray(itemRandom);
    }

    public void CheckOrder()
    {
     //   CustomerOrderManager.instance.CompleteOrder(currentSprayPreview.SprayData.id);
    }
    ...
    public override void SetUp(PhoneCase phoneCase)
    {
        ...
        UnSelect();
        if (sprayCam) sprayCam.targetTexture = phoneCase.CasePaintBinder.GetPaintRT();
        var a = phoneCase._paintRT;
        var b = sprayCam.targetTexture;
        Debug.Log($"A id={a?.GetInstanceID()}  B id={b?.GetInstanceID()}  sameRef={ReferenceEquals(a, b)}");

        var start = new Vector3(0f, axisPosition, 0f);
        if (lockAxis) start = ProjectToDragPlane(start);

        _toolTarget = start;
        if (sprayTool) sprayTool.position = start;
    ...
    public override void CompleteStep()
    {
      ...
      //  CustomerOrderManager.instance.HideItem();
        if (PlayerPrefs.GetInt("Tutorial") == 0)
        {
            UserTracking.TutorialAction(ActionTut.spray_end);
        }
    ...
    {
        sprayPreviews.Clear();
        var manager = ItemDataManager.Instance ? ItemDataManager.Instance.listSpray : null;
        if (manager == null || prayPreviewPrefab == null || spawnItem == null) return;

        foreach (var data in manager)
            if (data.isUnlock && !UserGameData.IsItemSprayUnlocked(data.id))
                UserGameData.UnlockItemSpray(data.id);

        for (int i = 0; i < manager.Count; i++)
        {
            var sprayCan = UnityEngine.Object.Instantiate(prayPreviewPrefab, spawnItem);
            sprayCan.SetUp(manager[i], () => { OnSelectSpray(sprayCan); });
            sprayCan.SetUpUnlock(UserGameData.IsItemSprayUnlocked(sprayCan.SprayData.id));
          ...
          /*  CallAdsManager.ShowRewardVideo("reward", () =>
            {
                ...
                if (sprayCanMat) sprayCanMat.mainTexture = p.SprayData.textureSprayCan;
                p.SetUpUnlock(true);
                currentSprayPreview = p;
                UserGameData.UnlockItemSpray(p.SprayData.id);
                if (_ps) Destroy(_ps.gameObject);
                if (_spray) Destroy(_spray.gameObject);

                _ps    = UnityEngine.Object.Instantiate(p.SprayData.brush, brushSlot.transform);
                _spray = UnityEngine.Object.Instantiate(p.SprayData.spray, effectSlot.transform);
                brush  = _ps.transform;
    ...
    public void UnSelect()
    {
        ...
        foreach (var spray in sprayPreviews) spray.Unselect();
    }
    ...
    private void HandlePointer(out bool hasPointer, out Vector3 pointer)
    {
        ...
        {
            tutorial.gameObject.SetActive(false);
            _lastInteractionTime = Time.unscaledTime; // tương tác mới
            StopDonePulse();                           // tắt nháy nếu đang chạy
            _armedIdlePulse = false;                   // hủy chờ idle
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            _lastInteractionTime = Time.unscaledTime;
            if (_sprayedDuringDrag) _armedIdlePulse = true;  // gài chờ idle
            _sprayedDuringDrag = false;
        }
